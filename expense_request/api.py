...
def make_journal_entry(expense_entry):
    ...
    if expense_entry.status == "Approved":         
        ...
        for detail in expense_entry.expenses:            
            ...
            expense_account_type = frappe.get_doc("Expense Entry Type", detail.expense_account)
            expense_account = ""

            for acc in expense_account_type.accounts:
                if(acc.company == expense_entry.company):
                    expense_account = acc.default_account 
        
            if not expense_account or expense_account == "":
                frappe.throw(
                    title="Error",
                    msg="The selected Expense Account has no linked account."
                )
        ...
        # if (expense_entry.mode_of_payment != "Cash" or and (not 
